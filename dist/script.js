  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function () {
      ...
      btns.forEach(function (item) {
        return item.classList.remove('active-style');
      });
...
var burger = function burger(menuSelector, burgerSelector) {
  ...
  window.addEventListener('resize', function () {
    if (window.screen.availWidth > 992) menu.style.display = 'none';
  });
};
...
var drop = function drop() {
  ...
  function highlight(item) {
    ...
    item.closest('.file_upload').style.backgroundColor = 'rgba(247, 231, 230, .7)';
  }

  function unhighlight(item) {
    item.closest('.file_upload').style.border = 'none';
    item.closest('.calc_form') ? item.closest('.file_upload').style.background = '#fff' : item.closest('.file_upload').style.background = '#ededed';
  }
...
var filter = function filter() {
  ...
  menu.addEventListener('click', function (e) {
    var target = e.target,
        markType = wrapper.querySelectorAll(".".concat(target.classList[0]));
    ...
    markType.length > 0 ? typeFilter(markType) : typeFilter();
  });
};
...
var scrolling = function scrolling(upSelector) {
  var upElem = document.querySelector(upSelector),
      ...
      body = document.body;
  window.addEventListener('scroll', function () {
    element.scrollTop > element.clientHeight * 2 ? upElem.classList.add('active') : upElem.classList.remove('active');
  }); // Scrolling with raf
  ...
  links.forEach(function (link) {
    link.addEventListener('click', function (e) {
      ...
      function step(time) {
        ...
        r !== withTop + toBlock ? requestAnimationFrame(step) : location.hash = hash;
      }
    });
  }); // Pure js scrolling
  // const calcScroll = () => {
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  var btn = document.querySelector(trigger); // cards.forEach(card => card.classList.add('animated', 'fadeInDown'))
  //
